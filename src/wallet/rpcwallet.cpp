...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Freecoin address: ")+name_);
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid freecoin address");
...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw runtime_error(
            ...
            "     \"totalFee\"          (numeric, optional) Total fee (NOT feerate) to pay, in lifetiparoshis.\n"
    ...
    if (totalFee > 0) {
        ...
        // add 1 lifetiparoshi to the result, because it may have been rounded down.

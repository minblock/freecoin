Freecoin Core version 0.13.2 is now available from:

  <https://download.freecoin.org/freecoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a freecoin
...
ECDSA signatures inside Freecoin transactions now use validation using
...
Previous versions of Freecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Freecoin Core was to raise the default minimum
relay fee.

Freecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
This allows running freecoind without having to do any manual configuration.
...
Freecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Freecoin Core will relay transactions with
...
entirely in Freecoin Core 0.13, and it is at this time undecided whether the
...
Freecoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Freecoin Core automatically creates a hidden service to listen on, without
manual configuration. Freecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Freecoin Core is listening, and
...
Freecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Freecoin
Core to use floating fees. By default, Freecoin Core will use floating
fees.
...
`freecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /freecoinrpc>
        ...
        # optional bypass freecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< freecoinrpc:<password>
    </Location>
...
Restarting freecoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in freecoin.conf
...
freecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/freecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Freecoin Core code base has
...
- `freecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `freecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Freecoin scripting system that in combination with
...
Freecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Freecoin Core.
...
The two phases are distinct in the Freecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Freecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Freecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Freecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of freecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of freecoins being spent is exactly
...
- **Increased security for multisig:** Freecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Freecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Freecoin's current ledger (called the UTXO set),
  ...
  note that Freecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Freecoin users to individually opt-in to almost any change in the Freecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Freecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Freecoin
client to Freecoin Core 0.13.2 to signal for segwit support. When a
...
Since Freecoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `freecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `freecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
